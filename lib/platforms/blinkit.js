...
async function searchViaProxy(query, lat, lng) {
  ...
  if (!ok) {
    ...
    if (/403|auth_key failed/i.test(err)) {
      throw new Error(
        "Blinkit blocked the cloud proxy IP. Run the Impit proxy on your home machine (where local works) and point BLINKIT_PROXY_URL at that tunnel — see services/blinkit-proxy/README.md."
      );
    }
    throw new Error(err);
  }
...
async function ensureAuthKey() {
  ...
  if (!ok || !data?.auth_key) {
    if (status === 403) {
      throw new Error(
        "Blinkit blocked this server. Set BLINKIT_PROXY_URL to the Impit proxy (e.g. Render) for Netlify."
      );
    }
