...
const CLOUD_UNAVAILABLE =
  "Flipkart Minutes needs a residential proxy on the live site (RESIDENTIAL_PROXY_URL), or run Compari5 locally with npm run dev.";
...
export async function searchMinutes(query, lat, lng, options = {}) {
  ...
  // Prefer optional home Impit tunnel only when ENABLE_HOME_PROXY=1.
  // Otherwise use direct Impit (local or via RESIDENTIAL_PROXY_URL).
